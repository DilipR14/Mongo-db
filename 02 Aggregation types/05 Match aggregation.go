...
func performGroupAggregationWithMatch(limit int, sortByField string, minAge int) ([]bson.M, error) {
    ...
    pipeline := mongo.Pipeline{
        bson.D{
            {"$match", bson.D{
                {"age", bson.D{
                    {"$gte", minAge}, // Filter documents where age is greater than or equal to minAge
                }},
            }},
        },
        bson.D{
...
func main() {
    ...
    aggregationResult, err := performGroupAggregationWithMatch(limit, sortByField, minAge)
    if err != nil {
        log.Fatal(err)
    }
